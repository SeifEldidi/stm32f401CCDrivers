 ...
 * 			HAL_OK  Success
 */
ERR xHAL_UsartSendNBYTESP(USART_Config * Usart , uint8_t *Buffer ,uint8_t Len)
{
	ERR Err_State = HAL_OK;
	if(Usart != NULL  && Buffer !=NULL)
	{
		...
		while(ByteTX < Len)
		{
			Err_State = xHAL_UsartSendBYTEP(Usart->Instance,*(Buffer+ByteTX));
			ByteTX++;
		}
	}else
		Err_State = HAL_ERR;
...
ERR xHAL_UsartRX1BYTESP(USART_Config * Usart , uint8_t *Buffer)
{
	ERR Err_State = HAL_OK;
	if(Usart != NULL  && Buffer !=NULL)
	{
	    while(!(Usart->Instance->SR & RXNE_READ));
		*Buffer = Usart->Instance->DR;
	}else
		Err_State = HAL_ERR;
	return Err_State;
}
