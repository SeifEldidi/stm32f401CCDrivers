...
static ERR HAL_PULL_SET(__IO GPIO_Typedef * GPIO_PORT,uint8_t PULL,uint8_t Shift)
{
...
static ERR HAL_Speed_SET(__IO GPIO_Typedef * GPIO_PORT,uint8_t Speed,uint8_t Shift)
{
	ERR Err_State = HAL_OK;
	switch (Speed) {
...
static ERR Hal_Gpio_Pin_Config(__IO GPIO_Typedef * GPIO_PORT, GPIO_InitStruct * GPIO_S,uint8_t Shift,uint8_t Pin_Num)
{
...
static ERR Hal_write_Pin(__IO GPIO_Typedef * GPIO_PORT,uint16_t GPIO_PIN,GPIO_State State)
{
	ERR Err_State = HAL_OK;
	switch(State)
	{
...
ERR  HAL_GPIO_Init(__IO GPIO_Typedef * GPIO_PORT, GPIO_InitStruct * GPIO_S)
{
	ERR Err_State = HAL_OK;
...
ERR  HAL_GPIO_WritePin(__IO GPIO_Typedef * GPIO_PORT,uint16_t GPIO_PIN,GPIO_State State)
{
	ERR Err_State = HAL_OK;
	if (GPIO_PORT == NULL )
		Err_State = HAL_ERR;
...
GPIO_State HAL_GPIO_ReadPin(__IO GPIO_Typedef * GPIO_PORT, uint16_t GPIO_PIN)
{
...
ERR  HAL_GPIO_TogglePin(__IO GPIO_Typedef * GPIO_PORT, uint16_t GPIO_PIN)
{
	ERR Err_State = HAL_OK;
	if (GPIO_PORT == NULL)
		Err_State = HAL_ERR;
