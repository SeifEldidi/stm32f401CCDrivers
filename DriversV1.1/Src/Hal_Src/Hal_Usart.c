...
static ERR Int_Request_Generation(USART_Typedef *Usart,USART_IRQ_Struct * Handler,uint8_t * data,uint32_t Len);
#endif
...
static int ConvIntegerToString(int32_t Val,char *String)
{
	uint8_t Cnt =0;
	uint8_t Len = 0 ;
	uint8_t Stack[10];
...
static void Usart1SendInterrupt(void)
{
	if(Usart1_Handler.Pos <Usart1_Handler.Len && *(Usart1_Handler.Buffer + Usart1_Handler.Pos)!='\0')
	{
		USART1->DR = *(Usart1_Handler.Buffer + Usart1_Handler.Pos);
		Usart1_Handler.Pos++;
	}else if(Usart1_Handler.Pos == Usart1_Handler.Len||*(Usart1_Handler.Buffer + Usart1_Handler.Pos)=='\0')
	{
		Usart1_Handler.State = USART_EMPTY;
		Usart1_Handler.Len = 0;
		Usart1_Handler.Pos = 0;
		Usart1_Handler.Buffer = NULL,
...
static void Usart2SendInterrupt(void)
{
	if (Usart2_Handler.Pos < Usart2_Handler.Len)
	{
		USART2->DR = *(Usart2_Handler.Buffer + Usart2_Handler.Pos);
		Usart2_Handler.Pos++;
	} else if (Usart2_Handler.Pos == Usart2_Handler.Len) {
		Usart2_Handler.State = USART_EMPTY;
		Usart2_Handler.Len = 0;
		Usart2_Handler.Pos = 0;
		Usart2_Handler.Buffer = NULL,
		/*----Clear Interrupt Enable on Usart---*/
		USART2->CR1 &= (~TCIE_EN);
	}
}
...
static void Usart6SendInterrupt(void)
{
	if (Usart6_Handler.Pos < Usart6_Handler.Len)
	{
		USART6->DR = *(Usart6_Handler.Buffer + Usart6_Handler.Pos);
		Usart6_Handler.Pos++;
	} else if (Usart6_Handler.Pos == Usart6_Handler.Len)
	{
		Usart6_Handler.State = USART_EMPTY;
		Usart6_Handler.Len = 0;
		Usart6_Handler.Pos = 0;
		Usart6_Handler.Buffer = NULL,
		/*----Clear Interrupt Enable on Usart---*/
		USART6->CR1 &= (~TCIE_EN);
	}
}
...
static ERR Int_Request_Generation(USART_Typedef *Usart,USART_IRQ_Struct * Handler,uint8_t * data,uint32_t Len)
{
	...
	if (Handler->State != USART_BUSY) {
		Handler->Buffer = data;
		Handler->Len = Len;
		Handler->State = USART_BUSY;
		Handler->Pos = 0;
		*USARTXTX_Callback = USARTXSend_Callback;
		/*-----Initiate Data Transfer----*/
		Usart->DR = *(Handler->Buffer + ((Handler->Pos)++));
		Usart->CR1 |= TCIE_EN;
	}else{
		return HAL_ERR;
	}
 ...
 * 			HAL_OK  Success
 */
ERR xHAL_UsartSendNBYTESP(USART_Typedef * Usart , uint8_t *Buffer ,uint8_t Len)
{
	ERR Err_State = HAL_OK;
 ...
 * 			HAL_OK  Success
 */
ERR xHAL_UsartSendString (USART_Typedef * Usart , uint8_t *Buffer )
{
	ERR Err_State = HAL_OK;
	if (Usart != NULL && Buffer != NULL) {
		uint32_t ByteTX = 0;
	...
	if(Usart != NULL  && Data !=NULL)
	{
		...
		while(*(Data + Str_Ptr) != '\0')
		{
			...
			if(*(Data + Str_Ptr) == '%')
			{
				...
				{
					char * Temp = (char *)va_arg(Ptr, void *);
 ...
 * 			HAL_OK  Success
 */
ERR xHAL_UsartSendBYTEAsync(USART_Typedef *Usart , uint8_t data)
{
	ERR Usart_State = HAL_OK;
	if (Usart != NULL ) {
		if (Usart == USART1) {
			Usart_State = Int_Request_Generation(USART1,&Usart1_Handler,&data,1);
		}else if(Usart == USART2)
		{
			Usart_State = Int_Request_Generation(USART2,&Usart2_Handler,&data,1);
		}else{
			Usart_State = Int_Request_Generation(USART6,&Usart6_Handler,&data,1);
		}
	} else {
		Usart_State = HAL_ERR;
	}
...
ERR xHAL_UsartSendNBYTESAsync(USART_Typedef * Usart , uint8_t *Buffer ,uint8_t Len)
{
	ERR Usart_State = HAL_OK;
	if(Usart != NULL  && Buffer != NULL)
	{
		if(Usart == USART1)
		{
			Int_Request_Generation(USART1,&Usart1_Handler,Buffer,Len);
		}else if(Usart == USART2)
		{
			Int_Request_Generation(USART2,&Usart2_Handler,Buffer,Len);
		}else if(Usart == USART6)
		{
			Int_Request_Generation(USART6,&Usart6_Handler,Buffer,Len);
		}
	}else{
		Usart_State = HAL_ERR;
	}
 ...
 * 			HAL_OK  Success
 */
ERR xHAL_UsartLogInfoAsync(USART_Typedef *Usart , uint8_t *Data ,...)
{
	ERR Err_State = HAL_OK;
	if(Usart != NULL  && Data !=NULL)
	{
		...
		while(*(Data + Str_Ptr) != '\0')
		{
			...
			if(*(Data + Str_Ptr) == '%')
			{
				...
				if(*(Data + Temp_ptr) == 'd')
				{
					if (IRQ_Handler->State == USART_EMPTY) {
						Str_Ptr++;
						Str_Ptr++;
						int32_t Temp = (int32_t) va_arg(Ptr, int);
						Len = ConvIntegerToString(Temp, String);
						xHAL_UsartSendNBYTESAsync(USART1, String, Len);
					}
				...
				}else if(*(Data + Temp_ptr) == 's')
				{
					if (IRQ_Handler->State == USART_EMPTY) {
						...
						Str_Ptr++;
						char * Temp = (char *)va_arg(Ptr, void *);
						xHAL_UsartSendNBYTESAsync(USART1, Temp,50);
					}
				}
			}else{
				if(IRQ_Handler->State == USART_EMPTY )
